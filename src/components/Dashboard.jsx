...
function DonutLabel({ viewBox, total }) {
    ...
    return (
        <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central">
            <tspan x={cx} y={cy - 8} style={{ fill: '#fff', fontSize: '1.5rem', fontWeight: 900, fontFamily: 'JetBrains Mono, monospace' }}>{total}</tspan>
            <tspan x={cx} y={cy + 14} style={{ fill: 'rgba(255,255,255,0.38)', fontSize: '0.56rem', letterSpacing: '0.12em', fontFamily: 'JetBrains Mono, monospace' }}>PENDING</tspan>
        </text>
    );
}
...
// ── Analytics Panel ────────────────────────────────────────────────────────────
function AnalyticsPanel({ tasks, allCategories, radarData, pieData }) {
...
export default function Dashboard({ userProfile, onReset }) {
    const storageKey = `dream_tasks_${userProfile?.name}`;
    const historyKey = `dream_history_${userProfile?.name}`;

    const [tasks, setTasks] = useState(() => {
        const saved = localStorage.getItem(storageKey);
        return saved ? JSON.parse(saved) : buildSmartTasks(userProfile);
    });
    ...
    const catStoreKey = `dream_cats_${userProfile?.name}`;
    ...
    useEffect(() => { localStorage.setItem(catStoreKey, JSON.stringify(customCategories)); }, [customCategories]);
    ...
    useEffect(() => {
        if (!remindersEnabled) return;
        const timer = setTimeout(checkAndNotify, 800); // slight delay after mount
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="header">
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    {[
                        ...
                        { id: 'custom', icon: <Settings size={15} />, label: 'Customize' },
                    ].map(btn => (
                    ...
                    <button onClick={toggleReminders}
                        title={remindersEnabled ? 'Reminders ON — click to disable' : 'Enable task reminders on this device'}
                        className="btn btn-outline"
                        style={{
                            padding: '0.55rem 0.7rem', borderRadius: '50%', position: 'relative',
                            borderColor: remindersEnabled ? 'rgba(251,191,36,0.6)' : undefined,
                            color: remindersEnabled ? '#fbbf24' : undefined,
                            boxShadow: remindersEnabled ? '0 0 16px rgba(251,191,36,0.2)' : undefined,
                        }}>
                    ...
                    <button onClick={onReset} title="Switch Profile" className="btn btn-outline"
                        style={{ padding: '0.55rem 0.7rem', borderRadius: '50%' }}>
                        <LogOut size={14} />
                    </button>
                </div>
            </div>
            ...
            <div className="glass-panel animate-fade-in" style={{
                ...
                background: remindersEnabled ? 'rgba(251,191,36,0.03)' : undefined,
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div>
                        <div style={{ fontSize: '0.82rem', fontWeight: 600, color: remindersEnabled ? '#fbbf24' : 'rgba(255,255,255,0.55)' }}>
                            {remindersEnabled ? '🔔 Reminders ON — fires every hour' : '🔕 In-App Reminders are OFF'}
                        </div>
                        <div style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.28)', fontFamily: "'JetBrains Mono',monospace", marginTop: '2px' }}>
                            Works on every device · no permissions needed
                        </div>
                    </div>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button onClick={sendTestNotification} className="btn btn-outline" style={{ fontSize: '0.78rem', gap: '0.35rem' }}>
                        <Bell size={13} /> Test Now
                    </button>
                    <button onClick={toggleReminders}
            ...
            {view === 'tasks' && (<>
                ...
                <div className="dashboard-grid">
                        ...
                        <div className="glass-panel">
                            ...
                            <div className="tasks-container" style={{ maxHeight: '420px', overflowY: 'auto', paddingRight: '6px' }}>
                                {filteredTasks.map(task => {
                                    const CatIcon = CATEGORIES[task.category]?.icon || Circle;
                                    const catColor = CATEGORIES[task.category]?.color || 'white';
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        ...
                        {/* Roadmap panel */}
                        <div className="glass-panel" style={{ borderLeft: '4px solid var(--accent-purple)', background: 'rgba(188,140,255,0.04)' }}>
                            <h3 style={{ color: 'var(--accent-purple)', marginBottom: '0.8rem', fontSize: '1rem' }}>🗺️ Roadmap Target</h3>
                            <p style={{ fontWeight: '700', fontSize: '1.05rem', marginBottom: '0.5rem' }}>{userProfile?.roadmap}</p>
                            <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', lineHeight: '1.6' }}>
                                Build 2+ projects, solve 100+ LeetCode, land an internship in this domain.
                            </p>
                            <div style={{ marginTop: '1rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '0.3rem' }}>
                                    <span>Progress</span>
                                    <span style={{ color: 'var(--accent-purple)', fontWeight: 'bold' }}>
                                        {tasks.filter(t => t.category === 'Projects' && t.completed).length}/{tasks.filter(t => t.category === 'Projects').length} Projects
                                    </span>
                                </div>
                                <div className="progress-bar" style={{ height: '7px' }}>
                                    <div className="progress-fill" style={{
                                        width: `${tasks.filter(t => t.category === 'Projects').length > 0
                                            ? Math.round((tasks.filter(t => t.category === 'Projects' && t.completed).length / tasks.filter(t => t.category === 'Projects').length) * 100)
                                            : 0}%`,
